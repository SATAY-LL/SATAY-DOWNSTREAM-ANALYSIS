...
import numpy as np
#
#def was used to do the things one three different datasets
#if you dont want to use the funtion, remove def.. & returm(..) and un# following line:
#readsforeverygenefile = 'data_step2_Dnrp1_1BREADS_exelled.csv'
def distribution_neutralgenes(readsforeverygenefile):
    neutralgenes = []
    positivegenes = []
    #in the following for loop, cellmap data is used to define if genes are neutral or positive
    ...
    #the following forloop collects the number of reads out of the datafile from the neutral and positive genes we just defined
    neutral_reads = [ ]
    positive_reads = []
    ...
    for i in range(0,len(tabel)):
        ...
        if tabel.at[i,"gene"] in neutralgenes and len(tabel.at[i,'reads'])>minimal_insertions:
            ...
            mean_per_gene_neutral.append(int(np.mean(tabel.at[i,"reads"]))) #compute average over each gene/change to np.median
        #positive
        elif tabel.at[i,"gene"] in positivegenes and len(tabel.at[i,'reads'])>minimal_insertions:
            positive_reads.append(tabel.at[i,"reads"])
            mean_per_gene_pos.append(np.mean(tabel.at[i,"reads"])) #compute average over each gene/change to np.median
       #all genes
        if  len(tabel.at[i,'reads'])>0:
            all_reads.append(tabel.at[i,"reads"])
            mean_all_reads.append(int(np.mean(tabel.at[i,"reads"]))) #compute average over each gene/change to np.median
    ...
    #it is necessary to create a flatlist
    #neutral
...
#do this for datafiles: output of allingment to yeastmine data.
